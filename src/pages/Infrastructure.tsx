...
import { Server, Cpu, HardDrive, Globe, Plus, Edit, Trash2, Search } from "lucide-react";
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="relative">
        ...
        <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar recurso, tipo, região ou provider..." className="bg-secondary/40 pl-9" />
      </div>

      {filtered.length === 0 ? (
        <div className="bg-card border border-border rounded-lg p-10 text-center">
          <Server className="h-10 w-10 text-muted-foreground/30 mx-auto mb-3" />
          <p className="text-sm text-foreground">Nenhum recurso cadastrado</p>
          <p className="text-xs text-muted-foreground mt-1">Adicione infraestrutura real para acompanhar custo, uso e renovação.</p>
        </div>
      ) : (
          ...
          return (
            <div key={r.id} className="bg-card border border-border rounded-lg p-4 card-hover cursor-pointer group">
              ...
              <div className="flex items-center justify-between mt-3 pt-2.5 border-t border-border/40">
                <span className="text-[10px] text-muted-foreground line-clamp-1">{r.notes ?? r.account_name ?? "Sem observações"}</span>
                <span className="font-mono text-[12px] text-foreground font-medium">${Number(r.monthly_cost).toFixed(0)}<span className="text-muted-foreground text-[10px]">/mês</span></span>
              </div>
            </div>
          );
        })}
      ...
      <Dialog open={!!deleteConfirm} onOpenChange={() => setDeleteConfirm(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Esta ação remove o ativo da base operacional.</DialogDescription>
          </DialogHeader>
          <DialogFooter>
      ...
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Conectado à base real de infraestrutura.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <Select value={form.asset_type} onValueChange={(value) => setForm((f) => ({ ...f, asset_type: value }))}>
                  <SelectTrigger className="bg-secondary/50"><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {['VPS','EC2','RDS','S3','Droplet','GKE'].map((item) => <SelectItem key={item} value={item}>{item}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="space-y-1.5"><Label className="text-xs">Custo mensal</Label><Input type="number" step="0.01" value={form.monthly_cost} onChange={(e) => setForm((f) => ({ ...f, monthly_cost: Number(e.target.value) }))} className="bg-secondary/50" /></div>
            </div>
            <div className="grid grid-cols-3 gap-3">
