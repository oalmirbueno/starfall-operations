...
const typeIcons: Record<string, any> = {
  vencimento: Clock, custo_alto: DollarSign, sem_responsavel: UserX, auto_renew_off: ToggleLeft, expiracao: AlertTriangle,
};
const urgencyLabel: Record<string, string> = { critico: "crítico", alto: "alto", medio: "médio", baixo: "baixo" };
...
export default function Alerts() {
  ...
  const goToSource = (a: any) => {
    if (a.subscription_id) navigate("/assinaturas");
    else if (a.infrastructure_asset_id) navigate("/infraestrutura");
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => computeAlerts.mutate()} disabled={computeAlerts.isPending} size="sm" variant="outline" className="gap-1.5">
          <RefreshCw className={`h-3.5 w-3.5 ${computeAlerts.isPending ? "animate-spin" : ""}`} /> Recalcular
        </Button>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {(["critico", "alto", "medio", "baixo"] as const).map(u => (
          <button key={u} onClick={() => setFilter(filter === u ? "all" : u)}
      ...
      {alerts.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-xs text-muted-foreground mb-4">
            {subscriptions.length === 0 ? "Adicione assinaturas primeiro" : "Todos os alertas foram resolvidos"}
          </p>
          {subscriptions.length > 0 && (
            <Button onClick={() => computeAlerts.mutate()} size="sm" variant="outline" className="gap-1.5"><RefreshCw className="h-3.5 w-3.5" /> Recalcular</Button>
          )}
        </div>
      ) : (
        <div className="space-y-3 canvas-stagger">
          {sorted.map(a => {
            ...
            return (
              <div key={a.id} className={`bg-card border rounded-lg p-4 flex items-start gap-4 card-hover ${
                a.urgency === "critico" ? "border-destructive/40" : a.urgency === "alto" ? "border-warning/30" : "border-border"
              }`}>
                <div className={`p-2 rounded-md shrink-0 ${
                  a.urgency === "critico" ? "bg-destructive/10" : a.urgency === "alto" ? "bg-warning/10" : "bg-secondary"
                }`}>
                  <Icon className={`h-4 w-4 ${
                    a.urgency === "critico" ? "text-destructive" : a.urgency === "alto" ? "text-warning" : "text-muted-foreground"
                  }`} />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-secondary text-muted-foreground">{a.type.replace("_", " ")}</span>
                  </div>
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  ...
                  <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-primary" title="Ir para origem"
                    onClick={() => goToSource(a)}>
                    <ExternalLink className="h-3.5 w-3.5" />
                  </Button>
                  <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-primary" title="Resolver"
      ...
      <div className="bg-card border border-border rounded-lg p-4 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[11px] text-muted-foreground">Motor de alertas ativo • E-mail e Telegram disponíveis</p>
        </div>
