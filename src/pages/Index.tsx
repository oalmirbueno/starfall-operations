  ...
  if (active && payload?.length) {
    return (
      <div className="bg-popover border border-border rounded-md px-3 py-2 text-xs shadow-lg">
        ...
        <p className="text-foreground font-bold font-mono text-sm">R$ {payload[0].value.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</p>
      </div>
    );
  }
  return null;
...
export default function Dashboard() {
  ...
  const progressPct = monthlyTotal > 0 ? Math.round((totalPago / monthlyTotal) * 100) : 0;
  ...
  return (
    <div className="space-y-5 canvas-stagger">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-0.5">Visão geral operacional em tempo real</p>
      </div>
            ...
            {monthlyTrend.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={monthlyTrend}>
                  ...
                  <YAxis axisLine={false} tickLine={false} tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 10, fontFamily: "JetBrains Mono" }} tickFormatter={v => `R$${v}`} width={60} />
                  <Tooltip content={<CustomTooltip />} />
                  <Area type="monotone" dataKey="total" stroke="hsl(var(--primary))" strokeWidth={1.5} fill="url(#grad)" />
                </AreaChart>
              </ResponsiveContainer>
            ) : (
              <div className="h-full flex items-center justify-center text-xs text-muted-foreground">Sem histórico suficiente.</div>
            )}
          </div>
        </div>
            ...
            {categoryBreakdown.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip formatter={(v: number) => `R$ ${v}`} contentStyle={{ background: "hsl(var(--popover))", border: "1px solid hsl(var(--border))", borderRadius: "6px", fontSize: "10px" }} />
                </PieChart>
              </ResponsiveContainer>
            ) : (
              <div className="h-full flex items-center justify-center text-xs text-muted-foreground">Sem categorias.</div>
            )}
          </div>
          <div className="space-y-1 mt-2">
            {categoryBreakdown.map(c => (
              <div key={c.name} className="flex items-center justify-between text-[11px]">
                ...
                <span className="font-mono text-foreground">R$ {c.value}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        <div className="bg-card border border-border rounded-lg p-4">
          ...
          <div className="space-y-2">
            ...
            {alerts.length === 0 && <div className="text-xs text-muted-foreground">Nenhum alerta ativo.</div>}
          </div>
        </div>
          ...
          <div className="space-y-2">
            {nextRenewals.slice(0, 5).map(s => (
              <div key={s.id} className="flex items-center justify-between p-2.5 rounded-md bg-secondary/40 border border-border/40 row-highlight">
                ...
                <span className="font-mono text-[13px] text-foreground">R$ {Number(s.value).toFixed(0)}</span>
              </div>
            ))}
              ...
              <div key={`${item.kind}-${item.id}`} className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="font-mono text-[13px] text-foreground">R$ {item.value.toFixed(2)}</span>
                  </div>
